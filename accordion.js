...
// accordion [20241222a]
const initializeAccordion = a => {
  const b = a.querySelectorAll('[data-accordion-header] button:not(:scope [data-accordion] *)');
  b.forEach(c => {
    ...
    const f = (a, b) => {
      ...
      e(a, b);
      if (b && c) {
        [...document.querySelectorAll(`[aria-expanded="true"][data-accordion-name="${c}"]`)].filter(b => {
          return b !== a;
        }).forEach(a => {
          e(a, false);
        });
